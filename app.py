...
def register():
    ...
    try:
        ...
        cur.execute("INSERT INTO users (username, password_hash) VALUES (?,?)", (d["username"], hash_pw(d["password"])))
        conn.commit(); conn.close()
        return {"ok": True}
    except sqlite3.IntegrityError: return {"error":"username exists"},400

@app.route("/login", methods=["POST"])
def login():
    d = request.json
    conn = db(); cur = conn.cursor()
    row = cur.execute("SELECT id FROM users WHERE username=? AND password_hash=?", (d["username"], hash_pw(d["password"]))).fetchone()
    conn.close()
...
def send_dm():
    d = request.json
    conn = db(); cur = conn.cursor()
    cur.execute("INSERT INTO private_messages (sender_id, receiver_id, message) VALUES (?,?,?)", (d["from"], d["to"], d["msg"]))
    conn.commit(); conn.close()
    return {"sent": True}
...
def dm_history():
    ...
    conn = db(); cur = conn.cursor()
    rows = cur.execute("SELECT sender_id,message,created_at FROM private_messages WHERE (sender_id=? AND receiver_id=?) OR (sender_id=? AND receiver_id=?) ORDER BY id",(a,b,b,a)).fetchall()
    conn.close()
...
def group_send():
    d = request.json
    conn = db(); cur = conn.cursor()
    cur.execute("INSERT INTO group_messages (group_id,sender_id,message) VALUES (?,?,?)",(d["group"],d["from"],d["msg"]))
    conn.commit(); conn.close()
    return {"sent":True}
...
def group_history():
    ...
    conn = db(); cur = conn.cursor()
    rows = cur.execute("SELECT sender_id,message,created_at FROM group_messages WHERE group_id=? ORDER BY id",(gid,)).fetchall()
    conn.close()
    return jsonify([dict(r) for r in rows])
